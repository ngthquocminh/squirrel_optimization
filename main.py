...
def setup_constraints(model: Model):
    ...
    for objtId, objt in model.objecttime_ids.items():
        ...
        model.add_constraint(model.sum(checkingVarList) >= minPeopleWorking)
        ...
        for thisShiftKey in getShiftKeys:
            ...
            for otherShiftKey in getShiftKeys:
                ...
                model.add_equivalence(
                    checkStart_var, thisShiftStart >= model.shift_start_vars[otherShiftKey]
                )
                model.add_equivalence(
                    checkEnd_var, thisShiftStart <= model.shift_end_vars[otherShiftKey]
                )
                arr = [checkStart_var, checkEnd_var]
                # for brk in range(0, MAX_BREAK_PER_SHIFT):
                #     _checkStart_var = model.binary_var()
                #     _checkEnd_var = model.binary_var()
                #     _key = (otherShiftKey[0], otherShiftKey[1], "{0}_br{1}".format(otherShiftKey[2],brk))
                #     _brkStart = model.break_start_vars[_key]
                #     _brkDuration = model.break_duration_vars[_key]
                ...
                #     model.add_equivalence(
                #         _checkEnd_var,
                #         thisShift >= _brkStart + _brkDuration
                #     )
            ...
            model.add_indicator(
                ...
                model.sum(checkingVarList) >= minPeopleWorking-1,
                name="MinPeopleWorking",
            )
...
def solve(model: Model, **kwargs):
    ...
    model.parameters.timelimit = 3600  # member should not take more than that !
